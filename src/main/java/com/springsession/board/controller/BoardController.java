...
public class BoardController {
                            ...
                            String searchKeyword)
    {
        ...
        int startPage=Math.max(nowPage-4,1); //처음 페이지
        int endPage=Math.max(nowPage+5,list.getTotalPages());  //마지막 페이지
    ...
    public String boardDelete(Integer id) {
        boardService.boardDelete(id);  //삭제 처리
        return "redirect:/board/list";
    }
    ...
    public String boardUpdate(@PathVariable("id") Integer id, Board board) {
        ...
        Board boardTemp=boardService.boardView(id); // 기존 데이터
